...
export default function AllFigure() {
  ...
  // Định nghĩa hàm fetchFigures
  const fetchFigures = async () => {
    try {
      const data = await FigureAPI.getAll();
      const fetchedFigures = data.data;
      setOriginalFigures(fetchedFigures); // Lưu trữ danh sách sản phẩm gốc
      setFigures(fetchedFigures);
      setShowNotFound(false); // Đặt lại trạng thái của biến showNotFound khi có dữ liệu
    } catch (error) {
