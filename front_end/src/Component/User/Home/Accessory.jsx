...
export default function Accessory() {
  ...
  useEffect(() => {
    async function fetchFigures() {
      try {
        const data = await FigureAPI.getAccessory();
        setFigures(data);
      } catch (error) {
          ...
          {figures.map((figure) => (
            <div className="slider-product-item" key={figure.id}>
              <Link className="home-product-item" to={`/Detail-Figure/${figure.id}`}>
                ...
                >
                  <span>{figure.quantity === 0 ? "Hết hàng" : "Giảm sốc"}</span>
                </div>
                <div className={`home-product-item__sale-off ${figure.promotionprice === 0 ? 'hidden' : ''}`}>
